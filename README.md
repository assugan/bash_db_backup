...
Скрипт выполняет резервное копирование всех пользовательских баз `PostgreSQL`, создаёт отдельные дампы, упаковывает их в архив, проверяет архив на целостность, перемещает его в каталог `/backups` и выполняет ротацию (хранит только последние N бэкапов).
